...
public class DesUtil {
     ...
     * @date: 2019/5/18
     */
    public static String getDesc(String text) {
        if (Strings.isNullOrEmpty(text) || !text.contains("*/")) {
            return null;
        }
        String[] menuList = text.split("\\*/")[0].split("@remark");
        if (menuList.length > 1) {
            return DesUtil.trimFirstAndLastChar(menuList[1].split("\\*")[0].replace("*", "").replace(":", "").replace("\n", " ").replace(" ", ""), ' ').trim();
        } else {
            return null;
     ...
     * @date: 2019/5/18
     */
    public static String getDescLink(String text) {
        if (Strings.isNullOrEmpty(text) || !text.contains("*/")) {
            return null;
        }
        String[] menuList = text.split("\\*/")[0].split("@link");
        if (menuList.length > 1) {
            return DesUtil.trimFirstAndLastChar(menuList[1].split("\\*")[0].replace("*", "").replace(":", "").replace("\n", " ").replace(" ", ""), ' ').trim();
        } else {
            return null;
     ...
     * @date: 2019/5/18
     */
    public static String getResultType(String text) {
        if (Strings.isNullOrEmpty(text) || !text.contains("*/")) {
            return null;
        }
        String[] menuList = text.split("\\*/")[0].split("@resultType");
        if (menuList.length > 1) {
            return DesUtil.trimFirstAndLastChar(menuList[1].split("\\*")[0].replace("*", "").replace(":", "").replace("\n", " ").replace(" ", ""), ' ').trim();
        } else {
            return null;
