...
public class BuildJsonForYapi {
    ...
    public static YapiApiDTO actionPerformed(PsiClass selectedClass, PsiMethod psiMethodTarget, Project project, PsiFile psiFile, String attachUpload) {
        ...
        String desc = DesUtil.getDesc(psiMethodTarget.getText());
        String link = DesUtil.getDescLink(psiMethodTarget.getText());
...
//        yapiApiDTO.setDesc(Objects.nonNull(yapiApiDTO.getDesc()) ? yapiApiDTO.getDesc() : " <pre><code>  " + classDesc + "</code></pre>");
        yapiApiDTO.setDesc("<pre><code>  " + desc + "</code></pre>");
        try {
            // 先清空之前的文件路径
            filePaths.clear();
            // 生成响应参数
            yapiApiDTO.setResponse(getResponseDesc(project, psiMethodTarget, psiMethodTarget.getText()));
     ...
     * @date: 2019/2/19
     */
    public static String getResponseDesc(Project project, PsiMethod psiMethod, String desc) throws JSONException {
        return getPojoJsonDesc(project, psiMethod, desc);
    }


    public static String getPojoJsonDesc(Project project, PsiMethod psiMethod, String desc) throws JSONException {
        ...
        if (psiType instanceof PsiPrimitiveType) {
        ...
        } else if (psiType.getPresentableText().startsWith("Response")) {
            KV result = new KV();
            String[] types = psiType.getCanonicalText().split("<");
            List<String> requiredList = new ArrayList<>();
            ...
            String link = DesUtil.getDescLink(desc);
            String resultType = DesUtil.getResultType(desc);
            PsiClass psiClass = JavaPsiFacade.getInstance(project).findClass(link.replace("}", ""),
            ...
            if (resultType.equals("list")) {
                ...
                for (int i = 0; i < psiFields.length; i++) {
                    KV datar = new KV();
                    datar.set("required", 0);
                    datar.set(KV.by("type", psiFields[i].getType().getCanonicalText()
                            .replace("java.lang.Long", "long")
                            .replace("java.lang.String", "string")
                            .replace("java.time.LocalDateTime", "date")
                            .replace("java.time.Date", "date")
                            .replace("java.lang.Integer", "number")));
                    String str = "";
                    if (null != psiFields[i].getDocComment()) {
                        str = psiFields[i].getDocComment().getText();
                    }
                    str = str.replace("/**", "")
                            .replace("*/", "")
                            .replace("*", "");
                    datar.set(KV.by("description", str));
                    datar.set("title", psiFields[i].getName());
                    requiredList.add(psiFields[i].getName());
                    listKv.set(psiFields[i].getName(), datar);
                }
                listKv.set(KV.by("type", "object"));
                KV items = new KV();
                ...
                data.set(KV.by("type", "object"));
                data.set(KV.by("description", "对象信息"));
                data.set(KV.by("properties", results));

                KV errCode = new KV();
                errCode.set(KV.by("type", "string"));
                errCode.set(KV.by("description", "错误码"));

                KV errMsg = new KV();
                errMsg.set(KV.by("type", "string"));
                errMsg.set(KV.by("description", "请求成功"));
                KV currentDate = new KV();
                currentDate.set(KV.by("type", "date"));
                currentDate.set(KV.by("description", "时间戳"));

                kvObject.set("errCode", errCode);
                kvObject.set("errMsg", errMsg);
                kvObject.set("data", data);
                kvObject.set("currentDate", currentDate);
                result.set("type", "object");
                result.set("required", requiredList);
                result.set("title", psiType.getPresentableText());
                result.set("properties", kvObject);
                result.set("description", "描述信息");
            } else {
                KV data = new KV();
                data.set("required", 0);
                data.set(KV.by("type", "object"));
                ...
                for (int i = 0; i < psiFields.length; i++) {
                    ...
                    datar.set(KV.by("description", str));
                    requiredList.add(psiFields[i].getName());
                    dataFor.set(psiFields[i].getName(), datar);
                }
                data.set("properties", dataFor);
                KV errCode = new KV();
                errCode.set(KV.by("type", "string"));
                errCode.set("required", 0);
                errCode.set(KV.by("description", "错误码"));

                KV errMsg = new KV();
                errMsg.set("required", 0);
                errMsg.set(KV.by("type", "string"));
                errMsg.set(KV.by("description", "请求成功"));
                KV currentDate = new KV();
                currentDate.set(KV.by("type", "date"));
                currentDate.set("required", 0);
                currentDate.set(KV.by("description", "时间戳"));
    ...
    public static String getPojoJson(Project project, PsiType psiType) throws JSONException {
        if (psiType instanceof PsiPrimitiveType) {
            ...
            PsiClass psiClassChild = JavaPsiFacade.getInstance(project).findClass(types[0], GlobalSearchScope.allScope(project));
            KV kvObject = getFields(psiClassChild, project, types, 1, requiredList);
            PsiField psiField = psiClassChild.findFieldByName("link", true);
            DesUtil.getLinkRemark("", project, psiField);

            KV v = getFields(psiClassChild, project, types, 1, requiredList);
            KV errCode = new KV();
            errCode.set(KV.by("type", "string"));
            errCode.set("required", 0);
            errCode.set(KV.by("description", "错误码"));
